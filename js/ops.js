...
//    var nameSplitted = afterSplit[0].trim().replace(/ \[.*\].*/g, '').split(' ');
//    this.name = nameSplitted[nameSplitted.length-1];
    ...
    this.loose = Number(afterSplit[fromIndex+4].replace(/\./g,'').replace(/\n.*/g, '').trim());
    ...
    this.print = function()
    {
        return this.lootDiff + " " + this.place + " " + this.name + " " + this.order + " " + this.level + " " + this.lootText + "\n";
    };
    ...
    this.createRow = function(row)
    {
            ...
            tbodyString += '</td><td>';
            tbodyString += "<center><button class=\"btnIgnore\" name=\"button\" id=\"button\" onclick=\"ignoreKnight(" + row +  ");\">x</button></center>";
            tbodyString += '</td>';
            return tbodyString;
    };
...
function dataPasted(textarea)
{
    ...
    if(console !== null)
        console.value = console.value + "splitted has " + splitted.length + " lines\n";
    //ranks.value = splitted[3];
    
    if(splitted.length !== 103 && splitted.length !== 107)
    {
         if(console !== null)
            console.value = console.value +"error of split\n";
         textarea.value = "";
         return;
    }
    
    var slip = 0;
    if(splitted.length === 103)
        slip = 3;
    else
        slip = 7;
    ...
    for (var i=0; i<100; i++)
    {
        //console.value = console.value + "creating knight from string " + splitted[slip+i] + "\n";
        var knight = new Knight(splitted[slip+i]);
        
        if(i === 98)
            lastPlace = knight.place;
        
        if(i === 99)
            knight.place = lastPlace+1;
        
        //console.value = console.value + "created knight " + knight.print();
        if(false === checkIfKnightInArrayAndUpdate(knight))
        {
                knightsArray[knightsArray.length] = knight;
        }
        //toPrint +=  knight.print();
    }
    
    knightsArray.sort(compare);
    //ranks.value = toPrint;
    //ranks.value = splitted;
...
function saveCheckpoint()
{
    var console = document.getElementById("console");
    if(console !== null)
        console.value = console.value + "Checkpoint saved\n";
    for (i=0; i<knightsArray.length; i++)
    {
    ...
    createTable();
    //printKnights(document.getElementById("rankTextArea"));
};
